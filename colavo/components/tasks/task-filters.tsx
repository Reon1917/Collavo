...
import { TaskImportance, TaskStatus, User } from "@/types";
        ...
        <div className="flex items-center space-x-2">
          ...
          {['title', 'deadline', 'importance', 'status'].map((field) => (
            <Button 
              ...
              className={sortOptions.field === field ? "bg-blue-600 text-white" : "text-gray-600"}
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <div className="space-y-1">
                <div 
                  className={`cursor-pointer px-2 py-1 rounded text-sm ${filters.assignee === null ? 'bg-blue-100 text-blue-800' : 'hover:bg-gray-100'}`}
                ...
                {users.map(user => (
                  <div 
                    key={user.id}
                    className={`cursor-pointer px-2 py-1 rounded text-sm ${filters.assignee === user.id ? 'bg-blue-100 text-blue-800' : 'hover:bg-gray-100'}`}
            ...
            <div>
              ...
              <div className="space-y-1">
                <div 
                  className={`cursor-pointer px-2 py-1 rounded text-sm ${filters.importance === null ? 'bg-blue-100 text-blue-800' : 'hover:bg-gray-100'}`}
                  onClick={() => handleImportanceChange(null)}
                >
                  All Levels
                </div>
                {['minor', 'normal', 'major', 'critical'].map(importance => (
                  <div 
                    key={importance}
                    className={`cursor-pointer px-2 py-1 rounded text-sm ${filters.importance === importance ? 'bg-blue-100 text-blue-800' : 'hover:bg-gray-100'}`}
            ...
            <div>
              ...
              <div className="space-y-1">
                <div 
                  className={`cursor-pointer px-2 py-1 rounded text-sm ${filters.status === null ? 'bg-blue-100 text-blue-800' : 'hover:bg-gray-100'}`}
                ...
                {['pending', 'in-progress', 'completed'].map(status => (
                  <div 
                    key={status}
                    className={`cursor-pointer px-2 py-1 rounded text-sm ${filters.status === status ? 'bg-blue-100 text-blue-800' : 'hover:bg-gray-100'}`}
